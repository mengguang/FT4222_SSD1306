...
static inline void ssd1306_WriteCommand(uint8_t byte) {
    ...
    i2c_buffer[0] = 0x00;
    i2c_buffer[1] = byte;
    ftStatus = FT4222_I2CMaster_Write(ftHandle, SSD1306_I2C_ADDR, i2c_buffer, 2, &sizeTransferred);
    if (FT_OK != ftStatus) {
...
static inline void ssd1306_WriteData(uint8_t *buffer, uint8_t buff_size) {
    ...
    ftStatus = FT4222_I2CMaster_Write(ftHandle, SSD1306_I2C_ADDR, i2c_buffer, buff_size + 1, &sizeTransferred);
    if (FT_OK != ftStatus) {
        printf("I2C master write error\n");
    }
    //printf("sizeTransferred: %d\n",sizeTransferred);
